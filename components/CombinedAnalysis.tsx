      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 md:px-8 py-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 text-sm">Complete assessment across all uploaded images</p>
          </div>
          <div className="hidden md:block bg-white/20 backdrop-blur-sm rounded-lg p-3">
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5 border border-blue-200">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-blue-700 uppercase tracking-wide">Total Damages</p>
              <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <p className="text-3xl font-bold text-blue-900">{uniqueDamages.length}</p>
            <p className="text-xs text-blue-600 mt-1">Unique damage points</p>
          </div>
          
          <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5 border border-green-200">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-green-700 uppercase tracking-wide">Total Cost</p>
            ...
            <p className="text-xs text-green-600 mt-1">Estimated repair cost (AUD)</p>
          </div>

          <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-5 border border-amber-200">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-amber-700 uppercase tracking-wide">Severity</p>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-xs text-amber-700">Severe:</span>
                <span className="text-lg font-bold text-red-600">{severeCount}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-amber-700">Moderate:</span>
                <span className="text-lg font-bold text-yellow-600">{moderateCount}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-amber-700">Minor:</span>
                <span className="text-lg font-bold text-green-600">{minorCount}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-5 border border-purple-200">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-purple-700 uppercase tracking-wide">Condition</p>
              <svg className="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <span
              className={`inline-block px-4 py-2 rounded-lg text-sm font-bold border-2 ${getConditionColor(
                overallCondition
              )}`}
            >
              {overallCondition}
            </span>
            <p className="text-xs text-purple-600 mt-2">Overall vehicle status</p>
          </div>
        </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(damageTypeGroups).map(([type, data]) => (
                <div key={type} className="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-2">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag, index) => (
                <span
                  key={index}
                  className="px-4 py-2 bg-blue-100 text-blue-700 rounded-lg text-sm font-semibold border border-blue-200 hover:bg-blue-200 transition-colors"
                >
                  {tag}
                </span>
              ))}
          ...
          <div>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
              {uniqueDamages.map((damage, index) => (
                <div
                  key={index}
                  className="border border-gray-200 rounded-xl p-5 bg-gradient-to-br from-white to-gray-50 hover:shadow-lg transition-all duration-200"
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <div>
                          <p className="font-bold text-gray-900">{damage.carPart}</p>
        ...
        {uniqueDamages.length === 0 && (
          <div className="text-center py-12 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border-2 border-green-200">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">No Damages Detected</h3>
            <p className="text-gray-600">All images show the vehicle is in excellent condition.</p>
          </div>
        )}
      </div>
    </div>
